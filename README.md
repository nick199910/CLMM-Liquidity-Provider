...
| Crate | Description |
|-------|-------------|
| **`amm-domain`** | Core entities, value objects, and pure mathematical logic (CLMM math, IL calc). |
| **`amm-simulation`** | Backtesting engine, price path generation, and position simulation. |
| **`amm-optimization`** | Strategy optimization algorithms (Grid Search, Genetic Algorithms). |
| **`amm-protocols`** | Adapters for Solana protocols (Raydium, Orca, Meteora). |
| **`amm-execution`** | Live execution engine, wallet management, and transaction building. |
| **`amm-data`** | Data ingestion, historical price fetching, and storage repositories. |
| **`amm-cli`** | Command-line interface for running simulations and optimizations. |
| **`amm-api`** | REST API for external integrations and dashboards. |
